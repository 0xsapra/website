...
We were given a java application [(Source code attached)](/website/assets/source/0ctf-hessian-onlyJdk) which takes user input as POST body and uses the `hessian` Library to parse the POST Input. The Vulnerability was straight forward to discover from the source : `Index.java`
```java
try {
...
1. The code `input.readObject()` means, whatever `Object` we send in the POST body, it will deserialize it and next the code calls `readObject` method on it and we as attacker wanna make sure we achive rce when code calls this method. 
...
fun main(args: Array<String>) {
    ...
    var gadget = createGadget() // This is the main gadget
...
#### 2. Setting up command executing function which will get us RCE
...
```kotlin
val myUiDefaultObject = UIDefaults().also { it.put("_", SwingLazyValue(...)) } 
```
