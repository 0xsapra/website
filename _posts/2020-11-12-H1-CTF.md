...
The theme of this h1-ctf was very interesting, they tried to mimic a fake company called GRINCH-NETWORKS.<br>
GRINCH-NETWORKS is a new Company rolling 1 new features on there website each day. We as hackers aim to penetrate each feature they release and find the flag as Proof of Concept(POC) that the fearure has bugs in it.

The challenge were very interesting ranging from recon, osint to advance exploit chains. My writeup for all the flags are listes below in the order 1 to 12
...
It was day-1, there werent any features/apps released yet. We are expecting the flag-1 hidden somewhere on the website. 
...
There was no description for this challenge but it was just continuation for the 1st challenge. 
...
Being challenge-2 this should also be something easy, after trying to look for flag and fuzzing sub-directories under `/s3cr3t-ar3a/` we can find the flag in the page source. We can do `INSPECT ELEMENT` on the page https://hackyholidays.h1ctf.com/s3cr3t-ar3a we will see the flag in one of the DIV TAG.

![](https://i.imgur.com/7ODWP27.png)

> FLAG-2 flag{b7ebcb75-9100-4f91-8454-cfb9574459f7}

> BONUS: This was good enough for me to move on to next challenge but there's a caveat. Doing a `curl https://hackyholidays.h1ctf.com/ | grep flag` didnt reveal the flag but its only visible in browser after doing `INSPECT-ELEMENT`. This means its being dynamically inserted into webpage by some JAVASCRIPT. Out of curiousity I thought of looking where this script is and it was inside the JQuery library at`https://hackyholidays.h1ctf.com/assets/js/jquery.min.js`. 
```javascript=
var h1_0 = 'la'
, h1_1 = '}'
, h1_2 = ''
...
This was a simple web app which hits an API endpoint to get list of users in paginated manner. 
So 
* https://hackyholidays.h1ctf.com/people-rater/page/1 -> first 5 users
* https://hackyholidays.h1ctf.com/people-rater/page/2 -> users from 5-10
...
...
So first step would be enumerate information about each user.

### Solution

I wrote a simple bash script to iterate on users from id [1-20]. Basically do a curl request for each userId from [1-20] `/people-rater/entry?id=BASE64({id:[1-20]})` and print the result
...
There was not much functionality to the app. We only have a login endpoint https://hackyholidays.h1ctf.com/swag-shop/api/login and nothing else. Trying to login with common username/password didnt work so brute-forcing credentials would be last step.

The next immediate thing to do is recon. Look for hidden directories under https://hackyholidays.h1ctf.com/swag-shop

### Fuzzing

Running ffuz under https://hackyholidays.h1ctf.com/swag-shop/FUZZ for finding hidden directories.

```bash=
>> sapra@MacBook-Pro ~ % fff -u https://hackyholidays.h1ctf.com/swag-shop/api/FUZZ -fc 404  -w ~/Desktop/hacking/wordlist/apiendpoints.txt
...
:: Progress: [3219/3219] :: Job [1/1] :: 111 req/sec :: Duration: [0:00:29] :: Errors: 0 ::
```

Great we find 2 intersting directories. 
* /sessions
    * This was open API which printed list all the login sessions that were logged in
    ![](https://i.imgur.com/y8chZXi.png)
    * Decoding base64 of each session we find bunch of `cookie` and 1 user-id
        ```javascript=
        var sessions=[
      ...
      console.log(sessions)
        ...
        ```
    * we have few cookies and an user-id `C7DCCE-0E0DAB-B20226-FC92EA-1B9043`
* /user
    * This api sent response 400(Missing required fields). We will need to fuzz this furthur for HTTP PARAMETER's.
    * Here we have a parameter missing so we will fuzz to guess the parameter now
        ```bash=
        >> sapra@MacBook-Pro $ fff -u "https://hackyholidays.h1ctf.com/swag-shop/api/user/?FUZZ=1"  -w ~/Desktop/parameters.txt -fs 35 -mc all
        ...
        ```
    * We have a parameter uuid.

### Solution

After finishing the recon we have an endpoint `/session` exposing some cookies and 1 user uuid and an endpoint `/user` which takes uuid as parameter. Hitting `/user` endpoint with uuid from /session we will get the flag
...
There is not much functionality in the application. We are persented with a login Form and description asks us to bypass it. First few thoughts would be sql-injection or something.
...
With this knowledge, it means we can enumerate `username` and `password`. If this doesnt work, we can go back to recon and search for hidden directories.
...
Ok, great we were able to find a valid username `access`. Now we can do the same thing for password.
...
We can notice logging in sets the cookie `securelogin=eyJjb29raWUiOiIxYjVlNWYyYzlkNThhMzBhZjRlMTZhNzFhNDVkMDE3MiIsImFkbWluIjpmYWxzZX0=` and we are logged in as non admin user. Base64 decode the cookie
```bash=
...
We are able to login as admin, but hey challenge aint over yet :P. 
...
ZIP's password is `hahahaha`. We can now decrypt the zip and it will output a flag.txt file which have the flag
```
...
Lets start by fuzzing the the parameter `template` in https://hackyholidays.h1ctf.com/my-diary/?template=FUZZ
...
Source code is pretty straight forward. Goal is to get contents of secretadmin.php by opening it as https://hackyholidays.h1ctf.com/my-diary/?template=secretadmin.php.
Problem is word `secretadmin.php` and `admin.php` are deleted from the user input template.
...
str_replace() will delete `admin.php` from input and leftover will be admin.php (ad~~admin.php~~min.php = `admin.php`) as str_replace does not recursively deletes the the word from the input. Using this knowledge we can use following payload to open `secretadmin.php` https://hackyholidays.h1ctf.com/my-diary/?template=secsecretadmadmin.phpin.phpretadmadmin.phpin.php
After deleting `admin.php` and `secretadmadmin.php` we will be left with `secretadmadmin.php`.
...
The web application allows us create Email templates to send to users. Without login a user can only create email template and `preview` how it will look but not save it. 
TL;DR We as user can create a temprory email template and `preview` it. Templates are generally a good candidate for tempalate-injection in which we can abuse variables of template to perform malicious activity. 
...
We need to find the flag location first. We can definately run `ffuf` but I prefer to manually understand the app structure first. 
We are provided with a demo email template for reference under `Guess what` Campaign as follows:
```
{{template:cbdj3_grinch_header.html}} Hi {{name}}..... Guess what..... <strong>YOU SUCK!</strong>{{template:cbdj3_grinch_footer.html}}
```

This `cbdj3_grinch_header.html` must be under some directory/file path. First thing that comes to mind is send a wrong html file and maybe error will show the file path. So lets try to create a template with random file path in search of error which probably leaks file path.

Create a new template with following data and preview it :
* Goto https://hackyholidays.h1ctf.com/hate-mail-generator/new to create new template
...
We can send following request to retrieve the flag.
```
preview_markup={{name}}&preview_data={"name":"{{template:38dhs_admins_only_header.html}}"}
```

`preview_markup` parameter does not contain `{{template:38dhs_admins_only_header.html}}`. we use substitution to add the admin's template.
...
It was a blogging app with few public blogs and few admin's only private blog. The end goal seems to read Admin's post which might contain the flag. Few things that come directly to mind is IDOR to fuzz and get Admin's blog ID but the authorization was handled properly.

So only thing I can think now is to get into admin's account. 


### Fuzzing/Recon

* So first obvious thing is to try default username/password and sql/nosql injection at https://hackyholidays.h1ctf.com/forum/login but it didnt work. 
* Directory bruteforcing on https://hackyholidays.h1ctf.com/forum/ gives 
    * `phpmyadmin` (Intersting but we dont have creds)
* Fuzzed for zipped sql file `db.zip`, `sql.zip` etc
* swap files `/.index.php.swp`, `/.index.swp` etc
... 
 

After fuzzing everything within website I cant find any credentials. It was time to look elsewhere. So I tried
* github
...
* Google Hacking
* certificates
* Create custom wordlist using https://github.com/digininja/CeWL and bruteforce logins.
...

Some of keywords I use to look are following:
* grinchnetworks (company name)
* hackyholidays.h1ctf.com (domain name)
* hackyholidays (sub-domain)
* Image Logo is : https://hackyholidays.h1ctf.com/assets/images/grinch-networks.png. `grinch-networks` is how they call. We should search `grinch-networks` 
* https://storage.googleapis.com/[grinch-networks/grinchnetworks]
...


After trying above recon tricks, I found a hit on github with word `grinch-networks`. Simply searching `grinch-network` on github will show us following repo https://github.com/Grinch-Networks/forum. 

Before downloading the blog itself, its always-always a good idea to check for older commits because usually they can contain admin removing credentials from uploaded code `OR` fixing a bug. In our case we can see that the commit 3rd commit https://github.com/Grinch-Networks/forum/commit/efb92ef3f561a957caad68fca2d6f8466c4d04ae was removing the credentials from the code. 
![](https://i.imgur.com/FGijYPh.png)

Now we have database credentials: `user:forum`, `pass:6HgeAZ0qC9T6CQIqJpD` and our fuzzing did show there was phpmyadmin. We can successfully login with credentials at https://hackyholidays.h1ctf.com/forum/phpmyadmin .
...
passwordHash is 32 character long meaning its md5. Goto https://crackstation.net/ and paste the admin's hash 35D652126CA1706B59DB02C93E0C9FBF there and it will crack it for us, the cracked hash is `BahHumbug`.

Login https://hackyholidays.h1ctf.com/forum/login with user `grinch` password is `BahHumbug`. We will be logged in as admin. Goto secret admin's post and get the flag at https://hackyholidays.h1ctf.com/forum/3/2
...
There is an admin login section to app at https://hackyholidays.h1ctf.com/evil-quiz/admin. 
...
* After creating user we are redirected to https://hackyholidays.h1ctf.com/evil-quiz/start. This page have certain questions to fill and submit.
* Submitting the answers to question redirect us to `score` page showing how many questions we got correct at https://hackyholidays.h1ctf.com/evil-quiz/score


We have 2 places where app takes our user input
1. username
2. quiz-answers

Trying basic sql-injection payload in username field, we can find that `username` field is vulnerable to 2nd order sql-injection. Lets POC first. 
* create user with name `1' or '1'='` at https://hackyholidays.h1ctf.com/evil-quiz/
* Goto `score` page(https://hackyholidays.h1ctf.com/evil-quiz/score), we will see: `There is 1212112 other player(s) with the same name as you!`

This confirms we have 2nd order injection. We can mimic the above behaviour with python scrip so that we dont have to do it manually.
...
Attaching script with detailed explaination as comments on how the attack works.

sol.py

After successfully exploiting the injection we will have following information:
* database name = `quiz`
...
There isnt really much functionality to application. We have a register and login user functionality. Trying sql-injection didnt work in either login or register. After registerting an user, we are redirected to https://hackyholidays.h1ctf.com/signup-manager without any functionality.
So basically dont have anything but regiserter/login/logout. 

### Fuzzing/Recon

Directory brute forcing https://hackyholidays.h1ctf.com/signup-manager/ we will get a `README.md` file(https://hackyholidays.h1ctf.com/signup-manager/README.md) with following content:
```
# SignUp Manager
...
The function creates a unique string by combining all the input from user (username, password, age, firstname & lastname) into single string and save it inside `users.txt`. Following pic depicts how the above function represts a user in users.txt file.

![](https://i.imgur.com/fU1E5jw.png)

The goal is clear, we need to set last byte which represents ADMIN to 'Y' instead of 'N'.

If we can insert 1-extra byte anywhere before `ADMIN` byte(last byte) and  set `lastname='NNNNNNNNNNNNNNY'` the overflow will cause value of `ADMIN='Y'` (each value shift's 1 byte right, last byte of lastname becomes ADMIN's byte value)

Most of the user input (username, password, firstname and lastname) are correctly stripped and constrained within 15 character. 

1 intersting property of integers in PHP is `100 === 1e2`. similary `1000 === 1e3`.

We have limit on age to be within 3 character and only 3 places are reserverd for age in user string. So sending something like `age=1e3` will evaluate age to 1000 (length=4, thus we get 1 extra character we needed). 
...
> There is also a URL on the solve page https://hackyholidays.h1ctf.com/r3c0n_server_4fdk59 which currently says `come back tomorrow`. Probably for next challenge

----
## Flag-11

~No Description~
We can get challenge URL after solving 10th challenge.
```
...
The app initially present us with Photo albums for year 2018,2019 and 2020. There are 1-3 images in each album.
...
* Chaning the image path will change the `auth` key in JSON and backend will error.
...
```
There is only 1 picture with album_id=2. Output of above query 1 picture. Thus we can confirm that we have 2 level injection. 

Now because we can control 2nd query results too, our goal is to SSRF to `/api/*` endpoint as found above. Following sql-injection should retrieve `photo` location like we want.
...
After running above script for a while with different directories, we will have hit for 2 API endpoints
```
...
We can write python script to brute force username and password as follows:

sol1.py

This gives `username = grinchadmin`, `password = s4nt4sucks`. 

We can login with these credentials at https://hackyholidays.h1ctf.com/attack-box/login and get the 11th flag
...
~No Description~
We can get challenge URL after solving 11th challenge.
```
...
Visiting https://hackyholidays.h1ctf.com/attack-box we are given 3 IP address and an option to attack them. 
...
Visiting attack url seems like it tries to DDOS the target url. Currently I wasnt really sure whats the end goal.

### Fuzzing/Recon

Trying Directory search doesnt reveal any hidden directory. Changing target IP address to some other value gives error. Trying injections in target and hash didnt work either. 

I tried github dorking, google hacking, s3 brute force and many other recon tricks none worked. 

Next immediate step I can think of trying to crack the hash. Having an ability to change target-ip should give us some ssrf. So lets try to crack the hash.
...
Ohk great, we have a hit. The `SECRET` used to create the hash is `mrgrinch463`. We can verify it as

```bash=
>> sapra@Mabook-pro $ echo -ne 'mrgrinch463203.0.113.33' | md5
5f2940d65ca4140cc18d0878bc398955 # this is same as original hash
```
...
* Goto https://lock.cmpxchg8b.com/rebinder.html . This service provide host with two 'A' Records which we can configure as required. So I configured them as follows:
...
* visit https://hackyholidays.h1ctf.com/attack-box/launch?payload=eyJ0YXJnZXQiOiI3ZjAwMDAwMS4wODA4MDgwOC5yYm5kci51cyIsImhhc2giOiIwMDgzMGVlN2U1N2Y3ZDgxY2U4ODliZDEyYTdjYjBiZCJ9 . It might not work on first go, so visit the URL multiple times. 
    * Reason being DNS rebinding works on logic of TOC-TOU(time of check, time of use). When checking the IP address against blacklist IP(127.0.0.1), the software makes a DNS query and gets IP=8.8.8.8.
    * This passes the blacklist check and do a orignal request to `7f000001.08080808.rbndr.us` which then resolves to IP=127.0.0.1
    * The attack is not 100% reliable because DNS server can send any IP for the host so we need to run it multiple times
    ![](https://i.imgur.com/5AIw6Ey.png)



On successfull exploitation we will be redirect to https://hackyholidays.h1ctf.com/attack-box/challenge-completed-a3c589ba2709 completion page with last flag.
