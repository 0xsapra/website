...
Being challenge-2 this should also be something easy, after trying to look for the flag and fuzzing sub-directories under `/s3cr3t-ar3a/` we can find the flag in the page source. Open `INSPECT ELEMENT` by pressing `F12` on windows or `Command+Option+i` on Mac, on the page https://hackyholidays.h1ctf.com/s3cr3t-ar3a we will see the flag in one of the DIV TAG.
