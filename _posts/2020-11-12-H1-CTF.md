...
Being challenge-2 this should also be something easy, after trying to look for the flag and fuzzing sub-directories under `/s3cr3t-ar3a/` we can find the flag in the page source. We can do `INSPECT ELEMENT` on the page https://hackyholidays.h1ctf.com/s3cr3t-ar3a we will see the flag in one of the DIV TAG.
...
* /sessions
    ...
    * Decoding base64 of each session we find bunch of `cookie` and 1 user-id
      ...
      console.log(sessions)
        ...
        ```
    * we have few cookies and a user-id `C7DCCE-0E0DAB-B20226-FC92EA-1B9043`
* /user
    * This API sent response 400(Missing required fields). We will need to fuzz this further for HTTP PARAMETER's.
...
The problem is that the word `secretadmin.php` and `admin.php` are deleted from the user input template.
...
str_replace() will delete `admin.php` from the input and leftover will be admin.php (ad~~admin.php~~min.php = `admin.php`) as str_replace does not recursively delete the word from the input. Using this knowledge we can use the following payload to open `secretadmin.php` https://hackyholidays.h1ctf.com/my-diary/?template=secsecretadmadmin.phpin.phpretadmadmin.phpin.php
...
This `cbdj3_grinch_header.html` must be under some directory/file path. The first thing that comes to mind is to maybe include an incorrect HTML file and maybe an error will show the file path. So let's try to create a template with a random file path in search of an error that probably leaks the file path.
...
We can send the following request to retrieve the flag.
```
...
* Directory brute forcing on https://hackyholidays.h1ctf.com/forum/ gives 
    * `phpmyadmin` (Interesting but we don't have creds)
* Fuzzed for zipped SQL file `db.zip`, `sql.zip` etc
* swap files `/.index.php.swp`, `/.index.swp` etc
... 
 

After fuzzing everything within the website I can't find any credentials. It was time to look elsewhere. So I tried
* GitHub
...
* Google Hacking
* certificates
* Create custom wordlist using https://github.com/digininja/CeWL and brute-force logins.
...

Some of the keywords I use to look are the following:
...
After trying the above recon tricks, I found a hit on GitHub with the word `grinch-networks`. Simply searching `grinch-network` on Github will show us the following repo https://github.com/Grinch-Networks/forum. 

Before downloading the blog itself, it's always always a good idea to check for older commits because usually, they can contain admin removing credentials from uploaded code `OR` fixing a bug. In our case, we can see that the commit 3rd commit https://github.com/Grinch-Networks/forum/commit/efb92ef3f561a957caad68fca2d6f8466c4d04ae was removing the credentials from the code. 
...
passwordHash is 32 characters long meaning it's md5. Go to https://crackstation.net/ and paste the admin's hash 35D652126CA1706B59DB02C93E0C9FBF there and it will crack it for us, the cracked hash is `BahHumbug`.
...
* After creating a user we are redirected to https://hackyholidays.h1ctf.com/evil-quiz/start. This page has certain questions to fill and submit.
...
We have 2 places where the app takes our user input
1. username
2. quiz-answers
